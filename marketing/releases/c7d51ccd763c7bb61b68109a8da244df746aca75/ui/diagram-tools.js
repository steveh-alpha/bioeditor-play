...
export function installDiagramTools({getDoc,getSelected,replaceDoc,add,modal,toast}) {
  ...
  function connect() {
    ...
    modal('Connect selected objects',`<p class="muted">The relationship follows these two objects as you move or resize them.</p><label for="relationship-direction">Direction</label><select id="relationship-direction"><option value="forward">${esc(nodes[0].label)} → ${esc(nodes[1].label)}</option><option value="reverse">${esc(nodes[1].label)} → ${esc(nodes[0].label)}</option></select><label for="relationship-style">Relationship</label><select id="relationship-style"><option value="arrow">Directed arrow</option><option value="inhibition">Inhibition bar</option><option value="line">Undirected line</option></select><button class="primary" id="create-relationship">Connect objects</button><p class="muted">The symbol describes your intended relationship; it does not verify a biological mechanism.</p>`);
