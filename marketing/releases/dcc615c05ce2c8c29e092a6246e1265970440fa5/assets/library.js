import {DNA_VERSION,DNA_PREVIOUS_VERSION,dnaGlyph} from './dna.js';
...
export const ASSETS=TECHNICAL_ASSETS.map(a=>a.id==='dna'?{...a,version:DNA_VERSION,previousVersion:DNA_PREVIOUS_VERSION,source:'BioEditor original double-helix schematic v0.3.3',modifications:'Smooth cubic backbones, paired bases, and explicit front/back layering replace the wire schematic.',limitations:'Conceptual double-helix schematic; not an atomic model. Base identity, sequence, strand direction, and major/minor groove dimensions are not represented. Scientific review pending.'}:a.id==='antibody'?{...a,version:ANTIBODY_VERSION,previousVersion:ILLUSTRATION_VERSION,source:'BioEditor original IgG schematic v0.3.1',limitations:'Conventional monomeric IgG schematic, not to scale. Glycans, intradomain disulfides, and subclass-specific detail are omitted.',references:[{title:'IgG subclasses and allotypes: from structure to effector functions',url:'https://doi.org/10.3389/fimmu.2014.00520',role:'Biological structure reference; no artwork reused'}]}:a).concat(IMMUNOGLOBULIN_ASSETS,THERAPEUTICS_ASSETS,VIRUSES_ASSETS);
export const isTechnicalArtwork=version=>[ILLUSTRATION_VERSION,ANTIBODY_VERSION,DNA_VERSION,DNA_PREVIOUS_VERSION,IMMUNOGLOBULIN_VERSION,THERAPEUTICS_VERSION,VIRUSES_VERSION].includes(version);
...
export function assetRecord(id,version) {
 ...
 if(id==='dna'&&version===DNA_VERSION)return ASSETS.find(a=>a.id===id);
 if(id==='dna'&&version===DNA_PREVIOUS_VERSION)return {...ASSETS.find(a=>a.id===id),version,previousVersion:ILLUSTRATION_VERSION,source:'BioEditor original double-helix schematic v0.3.2'};
...
export function glyph(kind,color,parameters,aspect=1,version) {
 ...
 if(kind==='dna'&&[DNA_VERSION,DNA_PREVIOUS_VERSION].includes(version))return dnaGlyph(color,version);
