import {createStorage,STORAGE_KEY} from './editor/storage.js';
import {finalizeEdit,centerNode} from './editor/edits.js';
import {installTrialGuide} from './ui/trial.js';
...
import {synchronizeConnections,removeNodes,duplicateNodes} from './editor/connections.js';
import {dimensions,rasterDimensions,preflight,attributionText,STYLES} from './studio.js';
...
const storage=createStorage(()=>window.localStorage),loaded=storage.load();
...
$('#canvas').addEventListener('pointerdown',e=>{
 ...
 if(!rotation&&!handle&&!target){selected.clear();render();return;}
...
document.addEventListener('keydown',e=>{
 ...
 if(mod&&key==='d'&&inEditor){e.preventDefault();action('duplicate');}
 if(mod&&key==='a'&&inEditor){e.preventDefault();selected=new Set(doc.nodes.map(n=>n.id));render();}
...
installTrialGuide({modal,toast,download,recover:recoveryOptions,getDoc:()=>doc,getSaveStatus:()=>saveStatus});
